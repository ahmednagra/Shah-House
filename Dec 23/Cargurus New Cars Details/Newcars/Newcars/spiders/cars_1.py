...
class NewcarsSpider(Spider):
    ...
    custom_settings = {
        ...
        'FEEDS': {
            f'output/demo_Cargurus New Cars Details.xlsx': {
                'format': 'xlsx',
                'fields': headers,
            }
        },
    ...
    def parse_newcars(self, response):
        ...
        try:
            ...
            cars = data.get('listings', [{}])
            # for dict in combine_dict:
            for car in cars:
                item = OrderedDict()
                car_id = car.get('id', 0)
                ...
                # if vin_number in self.current_scraped_items:
                #     print(vin_number, 'product already exist')
                #     continue
