...
class KoganSpider(Spider):
    ...
    def parse_category(self, response):
        try:
            ...
            self.errors_list.append(f"Error in parse_category function: {e}, URL: {self.get_unquoted_url(response.url)}")
    ...
    def parse_category_pagination(self, response):
        try:
            ...
            self.errors_list.append(f"Error in parse_category_pagination function: {e}, URL: {self.get_unquoted_url(response.url)}")
    ...
    def parse_product_detail(self, response):
        try:
            ...
            was_price = product_dict.get('current_was_price', 0.0) if product_dict.get('current_was_price', 0.0) is not None else ''
