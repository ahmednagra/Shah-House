...
class HyundaiOemSpider(Spider):
    ...
    def __init__(self, **kwargs):
        ...
        self.categories = []
        self.category_name = ''
        self.category_items_scraped_count = 0
        self.total_categories_count = 0
        self.category_total_products = 0   # Category Found Total Products at pagination
        self.current_scraped_item = []
        self.fitment_scraped_items = []
        self.scraper_items_count = 0
    ...
    def parse_categories_pagination(self, response):
        ...
        self.category_total_products += int(len(products))
        print(f'Found Products in Category: {self.category_name} and Product :{self.category_total_products}')
        self.mandatory_logs.append(f'\nCategory: {self.category_name} found total Product :{self.category_total_products}')
        ...
        for product in products:
            try:
                ...
                if url not in self.current_scraped_item:
                    ...
                    # test
                    self.category_items_scraped_count += 1
                    print(f'Category {self.category_name} items_scraped_count : {self.category_items_scraped_count}')
                    self.current_scraped_item= ['10', '4']
                    self.scraper_items_count += 1
    ...
    def spider_idle(self):
        ...
        if self.current_scraped_item:
            print(f'\n\nTotal {self.category_items_scraped_count}/ {self.category_total_products} items scraped from Category: {self.category_name}')
            self.mandatory_logs.append(f'\nTotal {self.category_items_scraped_count}/ {self.category_total_products} items scraped from Category: {self.category_name}\n')
        ...
        self.current_scraped_item = []
        self.category_items_scraped_count = 0
        self.category_total_products = 0
        self.fitment_scraped_items = []
    ...
    def close(spider, reason):
        ...
        spider.mandatory_logs.append(f'Spider "{spider.name}" Scraped Total Products are : "{spider.scraper_items_count}"\n\n')
