...
class AmazonProductSpider(Spider):
    ...
    def __init__(self):
        super().__init__()
        self.urls = self.read_input_urls_from_file('input/urls.txt')
        proxy_token = self.read_input_urls_from_file('input/proxy_key.txt')
        self.proxy = f"http://{proxy_token[0]}:@proxy.scrape.do:8080" if proxy_token else ''

    def start_requests(self) -> Iterable[Request]:
        for url in self.urls:
            yield Request(url=url, callback=self.product_detail, meta={'proxy': self.proxy})
    ...
    def read_input_urls_from_file(self, file_path):
        try:
