...
class TruckTradeSpider(Spider):
    ...
    def __init__(self, **kwargs):
        ...
        self.logs_filepath = f'logs/logs {self.current_date_time}.txt'
        self.script_starting_datetime = datetime.now().strftime("%d-%m-%Y %H:%M:%S")
        self.write_logs(f'Script Started at "{self.script_starting_datetime}"\n')

    def start_requests(self):
        # url = f"https://www.commercialtrucktrader.com/search-results-data?condition=U&year=2000:2001&page=1&priceFilterType=cash"
        ...
        for start_year in range(1900, 2025):
            ...
            yield Request(url=url,
                          callback=self.parse,
                          meta={'handle_httpstatus_list': [400, 401, 429], 'start_year': start_year,
                                'end_year': end_year})
    ...
    def parse(self, response, **kwargs):
        try:
            ...
            self.write_logs(f'Response json parse error{response.url}  error: {e}')
            return

        # year_values = data.get('facets', {}).get('year', [])[0].get('data', [])
        # if year_values:
        #     start_year = year_values[0].get('value', 0)
        #     end_year = year_values[1].get('value', 0)
        # else:
        #     start_year = response.meta.get('start_year', '')
        #     end_year = response.meta.get('end_year', '')
        start_year = response.meta.get('start_year', '')
        end_year = response.meta.get('end_year', '')

        total_pages = data.get('meta', {}).get('page', {}).get('total_pages', 0)
        total_trucks = data.get('meta', {}).get('page', {}).get('total_results', 0)
        self.all_vehicles_count += int(total_trucks)

        log = f"Year Ranges {start_year}: {end_year} has Total Vehicles {total_trucks} and Total Pages {total_pages}"
        self.write_logs(log)

        if total_trucks == 0:
            return

        trucks_dict = data.get('results', [])
        for truck in trucks_dict:
            try:
                condition = ''.join(truck.get('condition', {}).get('raw', [])[0:1])
                year = truck.get('year', {}).get('raw', 0)
                name = truck.get('description', {}).get('snippet', '')
                cat = ''.join(truck.get('category_name', {}).get('raw', [])[0:1])
                city = truck.get('city', {}).get('raw', '')
                state = truck.get('state_code', {}).get('raw', '')
                price = truck.get('price', {}).get('raw', '')
                image = ''.join(truck.get('photo_ids', {}).get('raw', [])[0:1])
                trim = truck.get('trim_name').get('raw', '')
                url = truck.get('ad_detail_url', {}).get('raw', '')

                item = OrderedDict()
                item['Title'] = f"{condition} {name} {cat}"
                item['Make'] = ''.join(truck.get('make_name', {}).get('raw', [])[0:1])
                item['Model'] = ''.join(truck.get('model_name', {}).get('raw', [])[0:1])
                item['Year'] = str(year).strip() if year else ''
                item['Vin'] = truck.get('id', {}).get('raw', '')
                ...
                item['Options'] = ''
                item['Mileage'] = ''
                item['Trim'] = trim if trim is not None else ''
                item['Color'] = ''
                item['Body Type'] = ''
                item['Fuel Type'] = truck.get('fuel_type').get('raw', '')
                item['Engine'] = ''
                item['Deal Rating'] = ''
                item['Days on Market'] = self.get_days_on_market(name=item['Title'],
                                                                 value=truck.get('create_date').get('raw', ''))
                item['Drive Train'] = ''
                item['City Fuel Economy'] = ''
                item['Highway Fuel Economy'] = ''
                item['Combined Fuel Economy'] = ''
                item['Dealer Name'] = truck.get('dealer_group_name', {}).get('raw', '').title()
                item['Dealer City'] = city
                item['Dealer State'] = state.title()
                item['Dealer Zip'] = ''
                item['Dealer Phone'] = self.get_dealer_phone_no(truck)
                item['Dealer Rating'] = ''
                item['URL'] = url
                ...
                self.write_logs(f'Error from Detail Vehicle Page Url: {url}, Error {e}\n')
                print(f'Error from Detail Vehicle Page Url: {url}, Error {e}\n')
                continue
    ...
    def parse_dealer_vehicles(self, response):
        ...
        for vehicle in vehicles:
            try:
                ...
                item['Highway Fuel Economy'] = ''
                item['Combined Fuel Economy'] = ''
                item['Dealer Name'] = vehicle.get('dealer_group_name', {}).get('raw', '').title() or response.meta.get(
                    'dealer_name', '').title()
                item['Dealer City'] = city
                item['Dealer State'] = state.title()
                item['Dealer Zip'] = ''
    ...
    def close(spider, reason):
        ...
        spider.write_logs(f'Spider "{spider.name}" was started at "{spider.current_date_time}"')
