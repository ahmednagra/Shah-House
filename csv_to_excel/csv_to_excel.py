...
class CSVtoExcelConverter:
    def __init__(self):
        self.input_file = os.path.join('input', '2019 to 2023 Gwinnett Tax Sale Results.csv')
    ...
    def process_csv_file(self):
        ...
        for row in data:
            item = {}
            item['Tax auction date'] = row.get('Tax Sale Date', '')
            item['Mailing address'] = row.get('Parcel', '')
            name = HumanName(row.get('OwnerName', ''))
            item['First Name'] = name.first
            item['Last Name'] = name.last
            item['Status'] = row.get('Status', '')
            item['Last sale price'] = row.get('Amount Due', '')
            items_list.append(item)

        return items_list

    def save_to_excel(self, items_list):
        field_names = {
            ...
            'Phone Status 2': '',
            'Phone Tags 2': '', 'Phone 3': '', 'Phone Type 3': '', 'Phone Status 3': '', 'Phone Tags 3': '',
            'Phone 4': '',
            'Phone Type 4': '', 'Phone Status 4': '', 'Phone Tags 4': '', 'Phone 5': '', 'Phone Type 5': '',
            'Phone Status 5': '',
            'Phone Tags 5': '', 'Phone 6': '', 'Phone Type 6': '', 'Phone Status 6': '', 'Phone Tags 6': '',
            'Phone 7': '',
            'Phone Type 7': '', 'Phone Status 7': '', 'Phone Tags 7': '', 'Phone 8': '', 'Phone Type 8': '',
            'Phone Status 8': '',
            'Phone Tags 8': '', 'Phone 9': '', 'Phone Type 9': '', 'Phone Status 9': '', 'Phone Tags 9': '',
            'Phone 10': '',
        ...
        try:
            workbook.save(self.get_output_file_name())
            print(f"Data saved to {self.get_output_file_name()}")
        except PermissionError:
            print("The output file cannot be saved because it is open. Please close the file and try again.")

    def convert_csv_to_excel(self):
        items_list = self.process_csv_file()
        if not items_list:
            return
        self.save_to_excel(items_list)
    ...
    def get_output_file_name(self):
        ...
        # Construct the output file name with the same base name and .xlsx extension
        output_file_name = f"{file_name_without_ext}.xlsx"
