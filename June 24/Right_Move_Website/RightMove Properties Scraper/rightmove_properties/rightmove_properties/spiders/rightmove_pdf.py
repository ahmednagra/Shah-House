...
class RightmoveSpider(Spider):
    ...
    custom_settings = {
        ...
        'FEEDS': {
            f'output/RightMove Properties {current_time}.xlsx': {
                'format': 'xlsx',
    ...
    def __init__(self, **kwargs):
        ...
        self.doc = None
        self.pdf_folder_path = f'output/PDF Files {self.current_time}'
        os.makedirs(self.pdf_folder_path)

    def start_requests(self):
        url = self.property_urls.pop(0)

        yield Request(url=url, headers=self.headers, meta={'handle_httpstatus_list': [410]})

    def parse(self, response, **kwargs):
        try:
            json_data = json.loads(response.css('script:contains("propertyData") ::text').re_first('= (.*)')).get(
                'propertyData', {})
        except:
            json_data = {}
        ...
        item['Property Type'] = self.get_value_by_heading(response, 'PROPERTY TYPE') or json_data.get('propertySubType',
                                                                                                      '')
        item['Bedrooms'] = self.get_value_by_heading(response, 'BEDROOMS') or str(json_data.get('bedrooms', ''))
        item['Bathrooms'] = self.get_value_by_heading(response, 'BATHROOMS') or str(json_data.get('bathrooms', ''))
    ...
    def get_property_urls_from_file(self):
        with open('input/property_urls.txt', mode='r', encoding='utf-8') as txt_file:
    ...
    def build_pdf_file(self, address):
        ...
        while os.path.exists(pdf_file):
            counter += 1
            pdf_file = os.path.join(self.pdf_folder_path, f'{slugify(address)}({counter}).pdf')
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        spider = super(RightmoveSpider, cls).from_crawler(crawler, *args, **kwargs)
    ...
    def spider_idle(self):
        if self.property_urls:
            url = self.property_urls.pop(0)
            print('Scrapy requested url for pdf output : ', url)

            self.crawler.engine.crawl(Request(url=url,
                                              callback=self.parse,
                                              dont_filter=True,
