...
class MysqlPipeline:
    ...
    def process_item(self, item, spider):
        ...
        for table_name in tables:
           ...
           # placeholders query = f"INSERT INTO `{table_name}` ({', '.join(valid_fields)}) VALUES ({placeholders})" \
           #          f" ON DUPLICATE KEY UPDATE {', '.join(f'{valid_fields} = VALUES({placeholders})' for field in valid_fields)}"
